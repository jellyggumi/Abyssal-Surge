#!/usr/bin/env python3
"""Validate immutable Cycle 005 Retrospective v1 JSON records with Pydantic v2."""
...
AUTHORIZED_RUN_ID = "20260716-stage5-rts-detail-cycle-005-v1"
...
class Claim(StrictRecord):
    claim_id: Annotated[str, Field(pattern=r"^C005-RETRO-\d{3}$")]
...
class NextAction(StrictRecord):
    action_id: Literal["C005-NEXT-001"]
    phase: Literal["stage5-content-depth-and-apk-research"]
    owners: Annotated[
...
class DetailDirectiveState(StrictRecord):
    det_rts: Literal["completed_verified"]
    det_stage: Literal["completed_verified"]
    det_res: Literal["completed_verified"]
    det_narr: Literal["verified_no_regression"]
    det_qa: Literal["passed"]
    det_doc: Literal["completed"]
...
class CycleRetrospectiveV1(StrictRecord):
    ...
    cycle_index: Literal[5]
    process_phase: Literal["P3-detail-execution"]
    ...
    decision_ids: Annotated[list[Literal["C005-D-001"]], Field(min_length=1, max_length=1)]
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(arguments) != 1:
        print("usage: cycle-005-retrospective-validator-v1.py RETROSPECTIVE.json", file=sys.stderr)
        return 2
