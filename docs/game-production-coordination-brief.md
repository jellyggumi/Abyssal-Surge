...
## Cycle 006 — Design alignment: world-bible naming + enemy ecosystem (2026-07-16)
- **Trigger (design audit → C006-D-001):** a frozen audit of shipped state vs design documents surfaced three findings. Run folder: `_workspace/20260716-stage5-rts-detail-cycle-006-v1/` (intake/design-audit.md + production/task-manifest.md, immutable).
  1. **Narrative title misalignment (HIGH):** shipped `STAGE_TITLES_LOCALIZED` used generic governance labels appearing nowhere in world-bible v2; the stage-continuity contract expects encounter beats to surface in play.
  2. **Enemy ecosystem absent from the lane (HIGH):** the Foe existed only as avatar + charge bar; no hostile presence ever crossed the one-directional battlefield.
  3. **Command↔lane representation gap (MEDIUM):** only STRIKE/BRACE spawned lane units; DISRUPT and RECOVER had zero battlefield representation.
- **DET6-TITLE:** the five world-bible encounter titles now surface in play — "The Bell Beneath Blackwater" / "The Quiet Standard" / "The Shore That Remembers" / "Names Under the Foam" / "The First Surge" (ko: 블랙워터 아래의 종 / 침묵의 군기 / 기억하는 해안 / 물거품 속의 이름들 / 첫 번째 서지) — live-swapping via the existing translateUI localization path; `N / 5 —` campaign prefix kept.
- **DET6-FOE:** when a Foe attack resolves in the real-time loop, hostile "Void Spawn" units spawn at the lane's right edge marching left — per-stage counts 1/1/2/2/3 (stages 1–5), speed = stage preset unitSpeed × 1.1, despawn at the left edge. Purely cosmetic presentation of already-resolved deterministic outcomes; no gameplay state mutation. Tactical monitor lists hostiles with a red-bordered row style ("VOID SPAWN" / ko "심연 스폰") and a separate hostile tally.
- **DET6-UNIT:** DISRUPT spawns a violet "Arcane Disruptor" lane unit (sprite `assets/images/unit_disrupt.png`, speed preset × 1.15); RECOVER triggers a transient emerald `recover-pulse` aura on the knight avatar (~800ms, no lane unit). Sprite sources + SVG fallbacks extended for DISRUPT and hostile VOIDSPAWN; monitor in-place updates cover all four unit kinds.
- **QA gate (DET6-QA, stop-ship veto):** both node fake-DOM tests pass; browser E2E extended (disruptor spawn during SURGE, hostile spawn + later despawn, encounter-1 title assert "The Bell Beneath Blackwater"); full unfiltered Playwright playtest and capture-live exit 0 with refreshed desktop+mobile screenshots; `retrospectives/cycle-006-p3-design-alignment-v1.json` validates.
- **Knowledge ops (DET6-DOC):** llm-wiki report `wiki/reports/2026-07-16-abyssal-surge-cycle-006-design-alignment.md` (index updated) plus this section.
## Cycle 007 — Deployment integrity, stage cinematics, APK path (2026-07-16)
- **Trigger (C007-D-001/002/003):** user mandate "게임 제작하고 배포까지해" plus a P0 audit finding. Run folder: `_workspace/20260716-stage5-rts-detail-cycle-007-v1/`.
- **P0 deployment defect (DET7-SW, resolved live):** two stacked issues kept real users on stale/broken builds — (1) `sw.js` was frozen at `abyssal-surge-v1` with cache-first core, so returning PWA users never received cycles 005/006; (2) the Pages workflow allowlist shipped ONLY the 5-file closure, so `sw.js`, `manifest.json`, `icon.svg`, and the entire `assets/` tree 404'd on the live site. Fixes: workflow guard/archive/inventory expanded to the full PWA closure (git ls-tree inventory + `node --check sw.js`); `sw.js` v3 with NETWORK-FIRST core (deploys propagate next load, offline falls back to cache), cache-first media, `skipWaiting`+`clients.claim`. Live-verified post-push: Pages run success, curl 200 across the closure, live `sw.js` shows v3, live `app.js` carries `BUILD_TAG "c007"`.
- **Stage cinematics (DET7-CINE):** no Atlas Cloud key exists in this environment, so the vox-director LOCAL methodology was applied — five ffmpeg zoompan Ken Burns cinematics over the stage backdrops (960×540, 24fps, 6.0s, CRF 30, visual-only), 1.01MB total against a 6MB budget (compresso quality-per-byte principle). `app.js` shows each once per NEW encounter with letterboxed overlay: click/keypress skip (capture-phase isolation — skip input never reaches gameplay), 7s hard timeout, immediate error fallback (no black screen), reduced-motion skip, and no replay on retry/resume. Correction to the cycle-006 note: hostile waves telegraph at charge-cycle START (arrival ≈ attack), not after resolution, and a successful DISRUPT dispels the wave.
- **Readability (DET7-READ):** hostile wave spawns flash the lane's right edge (250ms `lane-spawn-flash`; static tint under reduced motion) so stage-5's 2.5s tempo stays peripherally legible.
- **APK path (DET7-APK, honest-limitation):** TWA packaging kit shipped — `apk/twa-manifest.json`, `apk/assetlinks-template.json`, `apk/BUILD.md` including the origin-scoped verification warning (assetlinks.json must publish at the `jellyggumi.github.io` HOST ROOT via the user-site repo or a custom domain; otherwise the APK silently falls back to Custom Tabs); manifest.json gained rasterized `icon-512/192` PNGs. No Android tooling here, so no APK build is claimed.
- **QA gate (DET7-QA):** fake-DOM tests (incl. the updated Pages-closure pin test) pass; browser E2E exit 0 with new asserts — per-stage cinematic presence/src, skip isolation (focus unchanged), and a REAL service-worker integration check (`navigator.serviceWorker.ready`, controller claim, `caches.keys() === ["abyssal-surge-v3"]`, fresh `BUILD_TAG` across two SW-controlled reloads); capture-live exit 0; retrospective `cycle-007-p3-deployment-integrity-v1.json` validates (pydantic v2).
- **Worktree co-op incident:** a concurrent session shares this worktree (`_workspace/rts-hero-rework-20260716-a/`, Codex remote-control). 28 tracked cycle-004/005/006 records were found deleted unstaged and restored; cycle-007's own pre-commit governance files were later deleted and rewritten from the session transcript (recovery annotations inline). Source unknown — verified not this session. Protocol recorded in `production/worktree-coop-note-v2.md` + shared-memory `worktree-coop-protocol-20260716` v2; cycle 008 moves to a dedicated `git worktree`.
